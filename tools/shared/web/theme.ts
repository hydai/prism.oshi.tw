/**
 * The prism visual vocabulary shared by every Nova and Crystal page: dark-mode
 * variables, the CSS component set, the icon set, and the theme toggle.
 *
 * Only *generic* rules belong here. CSS that styles one worker's widgets (Nova's
...
/** The html.dark variable overrides every page needs. Worker-local widget CSS is appended by each worker. */
export const DARK_MODE_VARS_CSS = `
    html.dark {
      color-scheme: dark;
      --accent-pink: #F472B6;
...
/**
 * The prism site's visual vocabulary (glass shell, hero, chips, pills, rows,
 * cards, buttons, forms) expressed with the page CSS variables so dark mode
 * keeps working. Injected after the page's :root block and DARK_MODE_CSS.
 */
...
/** Lucide-style icon paths (24 grid, stroke 2, round caps/joins). */
...
/** Inline decorative icon; returns '' for unknown names so templates never break. */
export function svgIcon(name: IconName, size = 16, extraStyle = ''): string {
  const path = (ICON_PATHS as Record<string, string>)[name];
  if (!path) return '';
