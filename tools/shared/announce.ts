...
import { loadSecret, readJsonOr, repoRoot } from './cli.ts';

const ROOT = repoRoot();
const DEV_VARS_PATH = path.resolve(ROOT, 'admin/.dev.vars');
const PENDING_PATH = path.resolve(ROOT, 'data/.pending-announce.json');
...
export function loadAnnounceWebhook(): string | undefined {
  return loadSecret('DISCORD_WEBHOOK_ANNOUNCE', { devVarsPath: DEV_VARS_PATH });
}
...
const readFromRepoRoot = (source: string): string => fs.readFileSync(path.join(ROOT, source), 'utf-8');
...
export function readPendingBatches(pendingPath: string = PENDING_PATH): PendingBatch[] {
  const parsed = readJsonOr<{ batches?: PendingBatch[]; embeds?: DiscordEmbed[] }>(pendingPath, {});
