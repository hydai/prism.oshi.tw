-- Demo streamers for LOCAL development databases only.
-- Without them a fresh local database serves an empty /vod streamer dropdown
-- (and an empty /vod/api/streamers); run this right after `npm run db:migrate:local`.
-- Apply with: npm run db:seed:local   (wrangler d1 execute oshi-prism-nova --local --file=seed.sql)
-- Never run against production: oshi-prism-nova already holds these rows through
-- migrations 0003_seed_existing_streamers.sql and 0011_reseed_streamers.sql, and a
-- bootstrap-era row must not be resurrected as approved/enabled by re-seeding.
-- INSERT OR IGNORE keeps the file idempotent on a database that already has them.
