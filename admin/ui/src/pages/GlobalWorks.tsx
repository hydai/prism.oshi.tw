import { Fragment, useReducer, useState, type FormEvent } from 'react';
...
function useGlobalWorksController() {
  ...
  if (data !== loadedData) {
    setLoadedData(data);
    if (data) dispatch({ type: 'loadSucceeded', response: data });
  }
...
function Pagination({ controller }: { controller: GlobalWorksController }) {
  ...
  return (
    <SharedPagination
      ...
      shown={{ start: total === 0 ? 0 : (page - 1) * PAGE_SIZE + 1, end: Math.min(page * PAGE_SIZE, total) }}
      onPrev={() => dispatch({ type: 'previousPageRequested' })}
      onNext={() => dispatch({ type: 'nextPageRequested' })}
    />
  );
}
