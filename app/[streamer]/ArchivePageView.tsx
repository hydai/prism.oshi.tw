...
function ArchiveSidebar() {
  const {
    tagCounts, selectedTags, toggleTag,
...
function MobileSearchTab() {
  const { tagCounts, selectedTags, debouncedSearch, setDebouncedSearch, selectedArtist, setSelectedArtist, flattenedSongs, handlePlayFromFlattened } = useArchiveFilters();
  const { mobileTab, scrollContainerRef, setShowTagFilter } = useArchiveUi();
...
function TagFilterOverlay() {
  const { showTagFilter, setShowTagFilter } = useArchiveUi();
  const { selectedTags, clearTags, tagCounts, toggleTag } = useArchiveFilters();
