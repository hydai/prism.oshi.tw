...
export type TagScope = 'work' | 'performance';
...
export const TAG_DEFINITIONS: readonly TagDefinition[] = [
  { id: 'language:zh', label: '中文歌', category: 'language', aliases: ['中文', '華語', '國語', 'Chinese'], scope: 'performance', sortOrder: 10, active: true },
  { id: 'language:en', label: '英文歌', category: 'language', aliases: ['英文', 'English'], scope: 'performance', sortOrder: 20, active: true },
  { id: 'language:ja', label: '日文歌', category: 'language', aliases: ['日文', '日語', 'Japanese'], scope: 'performance', sortOrder: 30, active: true },
  { id: 'language:ko', label: '韓文歌', category: 'language', aliases: ['韓文', '韓語', 'Korean'], scope: 'performance', sortOrder: 40, active: true },
  { id: 'language:other', label: '其他語言', category: 'language', aliases: ['多語', 'Other language'], scope: 'performance', sortOrder: 90, active: true },

  { id: 'genre:pop', label: '流行', category: 'genre', aliases: ['Pop', 'J-Pop', 'C-Pop'], scope: 'work', sortOrder: 10, active: true },
  { id: 'genre:rock', label: '搖滾', category: 'genre', aliases: ['Rock'], scope: 'work', sortOrder: 20, active: true },
  { id: 'genre:folk', label: '民謠', category: 'genre', aliases: ['Folk'], scope: 'work', sortOrder: 30, active: true },
  { id: 'genre:jazz', label: '爵士', category: 'genre', aliases: ['Jazz'], scope: 'work', sortOrder: 40, active: true },
  { id: 'genre:electronic', label: '電子', category: 'genre', aliases: ['Electronic', 'EDM'], scope: 'work', sortOrder: 50, active: true },
  { id: 'genre:rap', label: '饒舌', category: 'genre', aliases: ['Rap', 'Hip-Hop'], scope: 'work', sortOrder: 60, active: true },
  { id: 'genre:rnb', label: 'R&B／靈魂樂', category: 'genre', aliases: ['R&B', 'Rhythm and blues', 'Soul'], scope: 'work', sortOrder: 70, active: true },

  { id: 'mood:ballad', label: '抒情', category: 'mood', aliases: ['Ballad', '慢歌'], scope: 'work', sortOrder: 10, active: true },
  { id: 'mood:upbeat', label: '歡快', category: 'mood', aliases: ['輕快', 'Upbeat'], scope: 'work', sortOrder: 20, active: true },
  { id: 'mood:healing', label: '療癒', category: 'mood', aliases: ['Healing', '放鬆'], scope: 'work', sortOrder: 30, active: true },

  { id: 'style:parody', label: '惡搞', category: 'style', aliases: ['搞笑', 'Parody'], scope: 'performance', sortOrder: 10, active: true },
  { id: 'style:acoustic', label: '自彈自唱', category: 'style', aliases: ['Acoustic', '不插電'], scope: 'performance', sortOrder: 20, active: true },
  { id: 'style:duet', label: '合唱', category: 'style', aliases: ['Duet', '多人合唱'], scope: 'performance', sortOrder: 30, active: true },
  { id: 'style:a-cappella', label: '清唱', category: 'style', aliases: ['A cappella', '阿卡貝拉'], scope: 'performance', sortOrder: 40, active: true },

  { id: 'source:anime', label: '動畫歌', category: 'source', aliases: ['動漫歌', 'Anime'], scope: 'work', sortOrder: 10, active: true },
  { id: 'source:vocaloid', label: 'Vocaloid', category: 'source', aliases: ['ボカロ', '虛擬歌手'], scope: 'work', sortOrder: 20, active: true },
  { id: 'source:game', label: '遊戲歌', category: 'source', aliases: ['Game music', '電玩'], scope: 'work', sortOrder: 30, active: true },
  { id: 'source:original', label: '原創曲', category: 'source', aliases: ['Original song', '原創歌'], scope: 'work', sortOrder: 40, active: true },
];
...
export interface TagCategoryGroup {
  category: TagCategoryDefinition;
  tags: TagDefinition[];
}

// Catalog-ordered IDs of the active tags one scope may carry.
export function activeTagIds(scope: TagScope): string[] {
  const ids: string[] = [];
  for (const tag of TAG_DEFINITIONS) {
    if (tag.active && tag.scope === scope) ids.push(tag.id);
  }
  return ids;
}

// Active tags of one scope in catalog order, grouped by category. Categories with nothing
// to offer are dropped so pickers and filters never render an empty group.
export function activeTagsByCategory(scope: TagScope): TagCategoryGroup[] {
  const tagsByCategory = new Map<TagCategoryId, TagDefinition[]>();
  for (const tag of TAG_DEFINITIONS) {
    if (!tag.active || tag.scope !== scope) continue;
  ...
  for (const category of TAG_CATEGORIES) {
    ...
    if (tags) groups.push({ category, tags });
  }
  return groups;
}
...
export function filterTagIdsByScope(tags: Iterable<string>, scope: TagScope): string[] {
  return normalizeTagIds([...tags].filter((tag) => getTagDefinition(tag)?.scope === scope));
}
...
export function validateTagSelectionForScope(
  value: unknown,
  scope: TagScope,
): TagSelectionValidation {
  const selection = validateTagSelection(value);
  if (!selection.ok) return selection;

  const wrongScope = selection.tags.filter((tag) => getTagDefinition(tag)?.scope !== scope);
  if (wrongScope.length > 0) {
    return {
      ok: false,
      error: `tag IDs are not valid for ${scope} scope: ${wrongScope.join(', ')}`,
    };
  }
  return selection;
}
