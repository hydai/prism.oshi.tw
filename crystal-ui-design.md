...
- Search-result quality is already reasonable server-side (scored LIKE search with title-prefix bonus — `tools/crystal/src/db.ts`), but the UI hides this. There is no live suggestion, no result count, no "why this matched" hint.
...
1. User arrives at `crystal.oshi.tw/`. No login, no onboarding. The page is a single centered column with a logo header, a tagline, and a glass form card. See `tools/crystal/src/form-page.ts` (container wrapper).
2. **Type selector** — 4-button grid. Default: `Bug 回報` is active. Labels: `Bug 回報 / 功能建議 / UI 問題 / 其他` (`form-page.ts`).
3. **Title** — single-line input, `maxlength="200"`, placeholder `簡短描述問題或建議` (`form-page.ts`).
...
5. **Body** — textarea, 5 rows, placeholder `請描述你遇到的問題或想要的功能…` (`form-page.ts`).
6. **Nickname** — optional, `maxlength="50"`, placeholder `選填，Q&A 公開回覆時顯示` (`form-page.ts`). Blank nicknames render as `匿名` on the Q&A.
7. **Public-reply toggle** — card-shaped checkbox, **checked by default** (`form-page.ts`). Label `允許公開回覆`; hint `勾選後你的問題與官方回覆將顯示在 Q&A 頁面`.
8. If the toggle is **unchecked**, the **contact** field slides open (animated `max-height` + `opacity`, 0.3s ease — `form-page.ts`). Contact becomes required. Placeholder `Email / Discord / Twitter 等，讓我們能回覆你`.
9. **Turnstile** widget (Cloudflare's human-verification challenge) with `data-theme="auto"` (`form-page.ts`).
10. **Submit** — full-width purple→blue gradient button `送出回報` (`form-page.ts`).
11. **Success** — green banner `感謝你的回報！編號：crys-xxxxxxxx`. Form resets, Bug type reactivated, contact hidden, Turnstile re-issues (`form-page.ts`).
12. **Error** — red banner with server-returned error messages joined by `、` (`form-page.ts`).
...
1. User arrives at `/qa` (via cross-link `查看 Q&A` from form, or directly). Centered column, slightly wider than form (720 px vs 640 px — `qa-page.ts`).
2. Same logo header, but wordmark reads `Crystal Q&A` (`qa-page.ts`). Subtitle: `已回覆的問題與建議`.
3. **Search** — a pill-shaped input with a leading magnifier icon and placeholder `搜尋問題…（按 Enter 搜尋）` (`qa-page.ts`). Implemented as a plain `<form method="get">` — press Enter to submit, full page reload. No live search, no type-ahead.
4. **Filter bar** — 5 pill anchors: `全部 / Bug / 功能建議 / UI / 其他` (`qa-page.ts`). Selected chip uses the purple→blue gradient. Clicking a chip navigates with the `type` query param preserved.
5. **Ticket cards** — a column with `gap: 16px`. Each card: type badge + status badge + `nickname · YYYY-MM-DD` (top row), title (h3), body (paragraph with `\n → <br/>`), and a purple-left-border `admin-reply` panel showing the curator's response and its date. See `qa-page.ts`.
6. **Pagination** — only rendered when there's more than one page. Row of 36×36 square buttons (`qa-page.ts`). Active page uses the gradient; others are frosted glass.
7. **Empty states** (`qa-page.ts`):
...
1. User starts typing the title on `/`. Debounce fires 250 ms after each keystroke (`form-page.ts, 400`).
2. Minimum query length: **2 chars if the query contains any CJK codepoint, else 3 chars** (`form-page.ts, 394-396`). This is the key localization rule — Chinese 2-character compounds (e.g. `登入`) are often meaningful and shouldn't be discarded like short Latin strings.
3. Below the threshold: nothing happens. Above: `fetch('/api/similar?q=…&limit=5')` with an `AbortController` that cancels any in-flight request from the previous keystroke (`form-page.ts`).
4. Server handles this at `tools/crystal/src/index.ts`, running the same scored LIKE search as the Q&A but in `public_all` scope — so **pending tickets show up**, not just replied ones. Response is lean (`id, type, title, status, replied_at, submitted_at`) — no body, no contact, no admin reply, for PII + payload reasons.
5. The panel renders inside the form, directly below the title input (`form-page.ts`). Header: `類似的既有回報 (N)` with an inline `隱藏` dismiss button.
6. Each row shows: type badge + status badge + truncated title.
7. **Interaction rule** (intentional, encoded in the client — `form-page.ts`):
...
8. The `隱藏` button sets a per-session dismissed flag so the panel stays hidden until the user edits the title again (`form-page.ts`).
...
### 4.1 Layout & chrome
- Document language: `zh-Hant` (`form-page.ts`).
- Font: **DM Sans** (Google Fonts, weights 400/500/600/700 + italic 400), preconnected (`form-page.ts`).
- Page background: **135° linear gradient**, `#FFF0F5` → `#F0F8FF` → `#E6E6FA` (lavender → alice blue → lavender), `background-attachment: fixed` (`form-page.ts`). In dark mode: `#0F0A1A` → `#0D1117` → `#0A0E1A` (`theme.ts`).
- Content container: **`max-width: 640px; margin: 0 auto; padding: 48px 16px`** (`form-page.ts`).
- Turnstile challenge script loaded asynchronously from `https://challenges.cloudflare.com/turnstile/v0/api.js` (`form-page.ts`).
- No-flash dark-mode detect script runs inline in `<head>` before body paint (`form-page.ts` + `theme.ts`).

### 4.2 Sections (top to bottom)
1. **Logo header** (`form-page.ts`)
   - 40×40 rounded tile with `radius-lg` (12px), gradient background `var(--accent-purple-light) → var(--accent-blue-light)`, containing a 22×22 white-stroke SVG of three stacked diamonds (layers icon, `M12 2L2 7l10 5 10-5-10-5z` + two more paths — `form-page.ts`).
   - Wordmark text `Prism Crystal`, 28 px, weight 700, letter-spacing `-0.5px`, purple→blue 135° gradient clipped onto the text (`form-page.ts`).
   - **Theme toggle** — absolutely positioned to the right of the header row (`form-page.ts`, `theme.ts`). See §6.7.
   - Tagline paragraph, 14 px, `--text-secondary`: `回報問題或建議新功能，幫助我們讓 Prism 更好`.

2. **Glass form card** (`form-page.ts`)
...
3. **Cross-links row** (`form-page.ts`)
...
4. **Footer tagline** (`form-page.ts`): 11 px, tertiary, centered — `Prism — 為你喜愛的 VTuber 打造歌單頁面`.

### 4.3 Form fields

| # | Field | Kind | Required | Maxlen | Placeholder | Default |
|---|-------|------|----------|--------|-------------|---------|
| 1 | `類型 *` | 4 toggle buttons (grid) | Yes | — | — | `bug` active (`form-page.ts`) |
| 2 | `標題 *` | `<input type=text>` | Yes | 200 | `簡短描述問題或建議` | empty |
| 3 | `詳細描述 *` | `<textarea rows=5>` | Yes | 5000 | `請描述你遇到的問題或想要的功能…` | empty |
| 4 | `暱稱` | `<input type=text>` | No | 50 | `選填，Q&A 公開回覆時顯示` | empty |
| 5 | `允許公開回覆` | checkbox (card-shaped) | — | — | label `允許公開回覆`; hint `勾選後你的問題與官方回覆將顯示在 Q&A 頁面` | **checked** (`form-page.ts`) |
...
| 8 | Submit | full-width button | — | — | label `送出回報` (`form-page.ts`) | idle |

Hidden payload fields added at submit time (`form-page.ts`):
- `context_url` — read from `?ref=` query param via `getContextUrl()` at `form-page.ts`. **Currently dormant** — the main site doesn't append `?ref=` on its Crystal links.
- `turnstile_token` — read from the `[name="cf-turnstile-response"]` hidden input the widget injects.

### 4.4 Validation

Client-side (HTML5 + JS):
- `required` attributes on title / body (`form-page.ts, 314`).
- `maxlength` enforced on title (200) and nickname (50).
- JS checks `cf-turnstile-response` exists before POSTing; if not, shows `請完成人機驗證` (`form-page.ts`).
...
| Turnstile verification failed | `驗證失敗，請重試` (from `index.ts`) |
| Network error | `網路錯誤，請稍後再試` (client catch at `form-page.ts`) |
| Unknown server error | `提交失敗` (fallback at `form-page.ts`) |
...
- The **status-pending** style is defined only in this page (not shared in `theme.ts`): text `#D97706`, border `rgba(217, 119, 6, 0.25)` light; `#FBBF24` / `rgba(251, 191, 36, 0.25)` dark (`form-page.ts`).

**Safety notes for the designer:** the client whitelists the `type` and `status` enum values before interpolating them into class names (`form-page.ts, 379-380`) and uses `textContent` (not `innerHTML`) for titles. Any redesign that introduces new status values or row layouts needs to preserve this defense.

### 4.6 Submit flow & result banner
- Submit button: full-width (`width: 100%`), `padding: 12px 24px`, radius-lg, no border, purple→blue 135° gradient, white text at 15 px weight 600, `box-shadow: 0 4px 14px rgba(139,92,246,0.25)` (`form-page.ts`).
...
- `#result` banner (`form-page.ts`):
  - Hidden by default.
  - **Success variant**: `bg #ECFDF5`, text `#065F46`, border `1px solid #A7F3D0`. Dark: `bg rgba(22, 163, 74, 0.10)`, text `#6EE7B7`, border `rgba(110, 231, 183, 0.20)` (`theme.ts`).
  ...
  - Success text is literally: `感謝你的回報！編號：crys-{8-hex}` (`form-page.ts`). IDs are generated as `crys-` + 8 random hex chars (`tools/crystal/src/db.ts`).
...
- Content container: **`max-width: 720px`** (wider than form by 80 px), `margin: 0 auto; padding: 48px 16px` (`qa-page.ts`).
- URL accepts three query params (`index.ts`): `type` (one of `bug|feat|ui|other`, else ignored), `page` (1-based, default 1), `q` (search string, trimmed). `limit` is fixed at 20 for the HTML page; the JSON twin `/api/qa` accepts `limit` up to 50 (`index.ts`).

### 5.2 Sections
1. **Logo header** — identical layout to form but wordmark text reads `Crystal Q&A` (`qa-page.ts`). Subtitle: `已回覆的問題與建議`.
2. **Search form** (`qa-page.ts`)
   ...
   - Input wrapper uses `position: relative` with a 16×16 magnifier SVG absolutely positioned `left: 14px` (`qa-page.ts`).
   - Input class `qa-search-input` — `radius: 999px`, `padding: 10px 16px 10px 40px`, `maxlength="100"`, `autocomplete="off"`, placeholder `搜尋問題…（按 Enter 搜尋）` (`qa-page.ts, 242-250`).
3. **Filter bar** — 5 pill anchors (`qa-page.ts`). See §6.4 for filter button styles.
4. **Ticket list** — flex column, `gap: 16px` (`qa-page.ts`). Empty states per §3 Journey B.
5. **Pagination** — only when `totalPages > 1` (`qa-page.ts`). Row of 36×36 square buttons, `gap: 6px`, centered.
6. **Cross-links** + **Footer** — `提交新回報 | 前往 Prism 歌單` (`qa-page.ts`), plus the shared tagline.

### 5.3 Ticket card anatomy (`qa-page.ts`)
...
- Anonymous nicknames render as literal `匿名` (`qa-page.ts`).
- Status label: `已回覆` (status=`replied`) or `已關閉` (status=`closed`) — `qa-page.ts`.
- **Admin reply panel** (`theme.ts`): `padding: 16px`, `radius-lg`, `border-left: 3px solid var(--accent-purple)`, `background: rgba(139, 92, 246, 0.06)` (light) / `rgba(192, 132, 252, 0.08)` (dark).

All user-supplied text passes through `escapeHtml()` (`qa-page.ts`) which escapes `& < > "` and converts `\n` to `<br/>`. The designer should assume multi-line bodies render as hard-wrapped paragraphs.

### 5.4 Empty states (`qa-page.ts`)
...
All tokens are declared as CSS custom properties. **Currently duplicated** across both pages — `form-page.ts` and `qa-page.ts`. Dark-mode overrides live in a single shared string (`DARK_MODE_CSS` at `theme.ts`) injected into both pages. A key redesign goal (G1) is consolidating this.

| Token | Light (hex) | Dark (hex) | Source |
|-------|-------------|------------|--------|
| `--accent-pink` | `#EC4899` | `#F472B6` | `form-page.ts` / `theme.ts` |
| `--accent-pink-light` | `#F472B6` | `#F9A8D4` | `form-page.ts` / `theme.ts` |
| `--accent-blue` | `#3B82F6` | `#60A5FA` | `form-page.ts` / `theme.ts` |
| `--accent-blue-light` | `#60A5FA` | `#93C5FD` | `form-page.ts` / `theme.ts` |
| `--accent-purple` | `#8B5CF6` | `#C084FC` | `form-page.ts` / `theme.ts` |
| `--accent-purple-light` | `#A78BFA` | `#D8B4FE` | `form-page.ts` / `theme.ts` |
| `--bg-page-start` | `#FFF0F5` (lavender blush) | `#0F0A1A` | `form-page.ts` / `theme.ts` |
| `--bg-page-mid` | `#F0F8FF` (alice blue) | `#0D1117` | `form-page.ts` / `theme.ts` |
| `--bg-page-end` | `#E6E6FA` (lavender) | `#0A0E1A` | `form-page.ts` / `theme.ts` |
| `--bg-surface-glass` | `#FFFFFF66` (white @ 40%) | `rgba(30,31,52,0.60)` | `form-page.ts` / `theme.ts` |
| `--bg-surface-frosted` | `#FFFFFF99` (white @ 60%) | `rgba(26,27,46,0.85)` | `form-page.ts` / `theme.ts` |
| `--text-primary` | `#1E293B` | `#E8EAF0` | `form-page.ts` / `theme.ts` |
| `--text-secondary` | `#64748B` | `#9CA3AF` | `form-page.ts` / `theme.ts` |
| `--text-tertiary` | `#94A3B8` | `#6B7280` | `form-page.ts` / `theme.ts` |
| `--border-default` | `#E2E8F0` | `rgba(255,255,255,0.10)` | `form-page.ts` / `theme.ts` |
| `--border-glass` | `#FFFFFF66` | `rgba(255,255,255,0.08)` | `form-page.ts` / `theme.ts` |
| `--border-accent-purple` | `#DDD6FE` | `rgba(192,132,252,0.25)` | `form-page.ts` / `theme.ts` |

**Hardcoded (non-tokenized) feedback colors** — redesign candidates for promotion to tokens (G1):

| Use | Light | Dark | Source |
|-----|-------|------|--------|
| Success banner bg / text / border | `#ECFDF5` / `#065F46` / `#A7F3D0` | `rgba(22,163,74,0.10)` / `#6EE7B7` / `rgba(110,231,183,0.20)` | `form-page.ts` + `theme.ts` |
| Error banner bg / text / border | `#FEF2F2` / `#991B1B` / `#FECACA` | `rgba(220,38,38,0.10)` / `#FCA5A5` / `rgba(252,165,165,0.20)` | `form-page.ts` + `theme.ts` |
| Status-pending text / border | `#D97706` / `rgba(217,119,6,0.25)` | `#FBBF24` / `rgba(251,191,36,0.25)` | `form-page.ts` |

### 6.2 Type & status badge colors (`theme.ts`)
...
Font family: **DM Sans** (loaded from Google Fonts with weights 400/500/600/700 + italic 400, opsz 9..40 — `form-page.ts`, `qa-page.ts`).
...
- **16 px** — main form card (`form-page.ts`).
- **12 px** — Q&A ticket cards (`qa-page.ts`).
- **8 px** — theme toggle button (`theme.ts`).

**Shadows:**
- Form card: `0 8px 32px rgba(0,0,0,0.06)` (`form-page.ts`).
- Ticket card: `0 4px 16px rgba(0,0,0,0.04)` (`qa-page.ts`).
- Submit button idle: `0 4px 14px rgba(139,92,246,0.25)` (`form-page.ts`).
- Submit button hover: `0 6px 20px rgba(139,92,246,0.3)` (`form-page.ts`).

### 6.7 Gradients

All linear, **135°** (top-left → bottom-right):
- **Page background:** `var(--bg-page-start)` → `var(--bg-page-mid)` (50%) → `var(--bg-page-end)` (`form-page.ts`).
- **Wordmark text (clipped):** `var(--accent-purple)` → `var(--accent-blue)` (`form-page.ts`).
- **Logo tile background:** `var(--accent-purple-light)` → `var(--accent-blue-light)` (`form-page.ts`).
- **Active button (type-btn active / filter-btn active / submit / pagination current):** `var(--accent-purple)` → `var(--accent-blue)` (`form-page.ts, 122` and `qa-page.ts, 182`).

### 6.8 Dark mode behavior

- No-flash detect script runs in `<head>` before body paint (`theme.ts`): reads `localStorage.theme`; if `'dark'` or (not set AND `prefers-color-scheme: dark`), adds `html.dark`.
- Toggle button (§6.9) persists the user's explicit choice to `localStorage.theme` (`'dark' | 'light'`).
- System preference change listener (`theme.ts`) auto-updates only when there is **no explicit override**.

### 6.9 Theme toggle (`theme.ts`)
...
- Positioned absolutely to the right of the logo row (`form-page.ts`, `qa-page.ts`).
...
### 7.3 Type selector + `.type-btn` (`form-page.ts`)
- Grid of 4 buttons, `repeat(4, 1fr)`, `gap: 8px`. Collapses to `repeat(2, 1fr)` at `max-width: 480px` (`form-page.ts`) — the ONLY media query in the app.
...
### 7.4 Form input / textarea (`form-page.ts`)
...
### 7.5 Form label + required mark + hint (`form-page.ts`)
...
### 7.7 Toggle row (checkbox-as-card) (`form-page.ts`)
...
### 7.8 Contact field (animated reveal) (`form-page.ts`)
...
- Toggled by the public-reply checkbox change handler (`form-page.ts`).
...
### 7.12 Filter bar + `.filter-btn` (`qa-page.ts`)
...
### 7.15 Pagination button (`qa-page.ts`)
...
|---------|-------|-------|--------|----------|--------|
| `.type-btn` | border → `--border-accent-purple` | (no explicit focus style) | purple→blue gradient + white | — | `form-page.ts` |
| `.filter-btn` | border + text → `--accent-purple` | (no explicit focus style) | purple→blue gradient + white | — | `qa-page.ts` |
| `.form-input` | (none) | border → `--border-accent-purple`, ring `0 0 0 3px rgba(139,92,246,0.1)` | — | — | `form-page.ts` |
| `.qa-search-input` | (none) | border → `--accent-purple`, ring `0 0 0 3px rgba(139,92,246,0.1)` | — | — | `qa-page.ts` |
| `.btn-submit` | `opacity: 0.92`, shadow deepens | (default browser) | (= idle) | `opacity: 0.5`, `cursor: not-allowed` | `form-page.ts` |
| `.cross-links a` | `opacity: 0.7` | — | — | — | `form-page.ts` / `qa-page.ts` |
| `.similar-link` | child `.similar-item` border → `--accent-purple` | — | — | — | `form-page.ts` |
| `.similar-dismiss` | text → secondary, bg `rgba(0,0,0,0.04)` / dark `rgba(255,255,255,0.06)` | — | — | — | `form-page.ts` |
| Theme toggle | (no explicit hover; transitions `background 0.2s`) | — | — | — | `theme.ts` |
| Pagination active page | (none defined) | — | — | — | `qa-page.ts` |
...
**What exists today:**
- Single explicit breakpoint at `max-width: 480px`: type selector collapses from 4 columns to 2 (`form-page.ts`).
- Filter bar uses `flex-wrap: wrap` so chips naturally reflow (`qa-page.ts`).
...
- Similar panel type labels (in JS enum map, `form-page.ts`): `Bug` · `功能建議` · `UI` · `其他`
- Similar panel status labels (in JS enum map, `form-page.ts`): `處理中` · `已回覆` · `已關閉`
...
- Ticket status labels (server-rendered, `qa-page.ts`): `已回覆` (replied) · `已關閉` (closed) — only these two since Q&A scope excludes `pending`
...
11. **The `crys-XXXXXXXX` ID** (G2). 8 random hex chars (`db.ts`). Short and memorable but never shown anywhere after the success banner. **Q:** Keep format but show it more prominently; offer a copy button; put it in the URL if permalinks land (#8).
...
- `form-page.ts` — submission page: HTML + inline CSS + client JS (586 lines)
- `qa-page.ts` — Q&A page: HTML + inline CSS (301 lines)
...
### Key deep links
- CSS token block (light mode): `form-page.ts` / `qa-page.ts`
- Dark-mode tokens: `theme.ts`
- Dark-mode no-flash detect script: `theme.ts`
- Badge styles (shared): `theme.ts`
- Admin-reply panel styles: `theme.ts`
- Glassmorphism — form card: `form-page.ts`
- Glassmorphism — ticket card: `qa-page.ts`
- Similar-panel CSS: `form-page.ts`
- Similar-panel client logic (debounce, abort, render): `form-page.ts`
- Similar-panel interaction rule (linked vs inert): `form-page.ts`
- Submit-flow client logic: `form-page.ts`
- Form-input + focus ring: `form-page.ts`
- Submit button: `form-page.ts`
- Type selector + active state: `form-page.ts`
- Toggle row: `form-page.ts`
- Contact reveal animation: `form-page.ts`
- Result banner success/error hex: `form-page.ts` + `theme.ts`
- Single media query (480 px): `form-page.ts`
- Q&A ticket card template: `qa-page.ts`
- Q&A search + filter bar: `qa-page.ts, 234-261`
- Q&A empty states: `qa-page.ts`
- Pagination buttons: `qa-page.ts`
- Search ranking constants: `db.ts` (PREFIX_SCORE=10, TITLE_SCORE=3, BODY_SCORE=1, REPLY_SCORE=1, MAX_TOKENS=4, MAX_Q_LEN=100)
- `/api/similar` handler + CJK min-chars rule: `index.ts`
- Validation error copy: `validate.ts`
- ID generator (`crys-XXXXXXXX`): `db.ts`
